//NAME-VASU MEHTA//
//STUDENT ID-200554999//
$(document).ready(function () {
    const apiKey = 'cX3FWC/kM+Y1ImyLtvv9GA==G8AUwDkmiRtEwbBb'; // Replace 'YOUR_API_KEY' with your actual API key
    ...
    $('#get-recipe-btn').click(function () {
        // Clear previous recipe if any
        $('#recipe-container').empty();

        // Get a random recipe from the predefined list
    ...
    function displayRecipe(recipe) {
        // Create HTML to display the recipe
        const recipeHTML = `
            <h2>${recipe.title}</h2>
            <div class="recipe-details">
                <p><strong>Ingredients:</strong> ${recipe.ingredients}</p>
                <p class="servings"><strong>Servings:</strong> ${recipe.servings}</p>
                <p class="instructions"><strong>Instructions:</strong> ${recipe.instructions}</p>
            </div>
        `;
        
        // Append the recipe HTML to the container
